...
def upload_file():
    ...
    for file in os.listdir('images'):
        if file.endswith('.pdf'):
            continue
        if gemini_call_for_math_detection('images/' + file):
            print("this file was passed by")
            continue
        prompt_parts.append( f"""\"input: \",
        genai.upload_file(images/{file}),\"""")

    result_string = getGeminiResponse(prompt_parts=prompt_parts)
...
def gemini_call_for_math_detection(file_path):
    ...
    prompt_parts = [
        ...
        "give a binary response to the image 0 if it does not contain any math formulas and 1 if it does contain math formuala",
        "output: ",
        ]
...
def getGeminiResponse(prompt_parts):
    """Analyzes a math image using Gemini Pro Vision and returns LaTeX expressions."""
    print("making Gemini for expressions")
    model = genai.GenerativeModel('gemini-1.5-pro')
    generation_config = {
        "temperature": 1,
        "top_p": 0.95,
